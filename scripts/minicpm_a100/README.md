...
- **content blocks**：图片含内容语义与多轮顺序断言；标准 `input_audio` 块直接对目标转录计算 WER，使用 `librispeech_clean_wer` 门槛，错误率与阈值保存在请求 JSONL。单条音频仅为回归检查，完整 ASR 基准仍另行运行。
- **恢复与取消证据**：空输出和主动断流后均发送文本、语音请求；语音需通过波形校验。取消请求使用唯一 request_id，并在 10 秒内查找对应的 `Coordinator aborted` 日志。只有客户端提前断流、协调器 abort 日志与后续请求均符合要求才通过；缺少取消证据记为 BLOCKED，不将“客户端关闭连接”当作“服务端已取消”。日志证明协调器广播取消并更新状态，不证明所有 stage 已释放 GPU 资源。
- **流式响应**：与非流式一致执行 `contains` 文本断言，保存 `finish_reason` 并计入截断统计；主动取消不要求收到 `[DONE]`。截断本身作为统计记录，是否失败取决于该用例的内容断言。
- **性能比较**：基线、A/A、A/B 必须均为 PASS，并包含配置要求的全部测量组合、轮次和成功请求数，否则解释固定为 `inconclusive`，同时列出不符合条件的实验。
